...
type handler struct {
	svcService service.Category
}

func NewHandler(svcService service.Category) rhandler.Category {
...
func (h *handler) GetByGUID(w http.ResponseWriter, r *http.Request) {
	...
	category, err := h.svcService.GetByGUID(r.Context(), guid)
	if err != nil {
...
func (h *handler) Update(w http.ResponseWriter, r *http.Request) {
	...
	resp := entity.RequestCategoryUpdate{
		Name: category.Name,
	}
...
func (h *handler) List(w http.ResponseWriter, r *http.Request) {
	categories, err := h.svcService.List(r.Context())
	if err != nil {
		if errors.Is(err, entity.ErrNotFound) {
			httph.SendError(w, r, http.StatusNotFound, err)
			return
